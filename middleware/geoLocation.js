...
export const getNewGeo = async( req, res, next)=>{
    ...
    if(req.body.city){
        ...
        try {
            const response =  await axios.get(`https://api.opencagedata.com/geocode/v1/json?q=${city}&key=${process.env.OCD_API_KEY}&language=en&pretty=1`)
...
export const getCity = async (latitude,longitude)=>{
    ...
    try {
        ...
        const response =  await axios.get(`https://api.opencagedata.com/geocode/v1/json?q=${geoPoint}&key=${process.env.OCD_API_KEY}&language=en&pretty=1`)
